...
router.post('/create', async (req, res, next) => {
  try {
    ...
    // BEFORE restarting fail2ban, check if a jail with this filter name exists
    // If it exists and is disabled, enable it in config FIRST
    // This way we only restart fail2ban once
    let jailAutoStarted = false;
    let jailEnabled = false;
    try {
      ...
      if (jailsList.includes(name)) {
        ...
        if (jailConfig) {
          ...
          // If jail is disabled, enable it in config BEFORE restarting fail2ban
          if (jailConfig.config.enabled === false || jailConfig.config.enabled === 'false') {
            ...
            try {
              ...
              console.log(`[FILTER CREATE] ✅ Jail enabled in config: ${enableResult.configFile}`);
              jailEnabled = true;
            } catch (enableErr) {
              console.warn(`[FILTER CREATE] ⚠️ Failed to enable jail in config: ${enableErr.message}`);
              // Continue anyway - will try to start jail after restart
            }
          } else {
            // Jail is already enabled
            jailEnabled = true;
            console.log(`[FILTER CREATE] Jail "${name}" is already enabled in config`);
          }
        } else {
          console.warn(`[FILTER CREATE] ⚠️ Jail "${name}" found in configured jails but could not read config`);
        }
      } else {
        console.log(`[FILTER CREATE] No jail found with name "${name}" - filter created but jail not auto-started`);
      }
    } catch (jailCheckErr) {
      console.warn(`[FILTER CREATE] ⚠️ Could not check for jail before restart: ${jailCheckErr.message}`);
    }

    // Restart fail2ban service (mandatory - filters are only loaded at service boot)
    // This will also load the updated jail config if we enabled it above
    ...
    try {
      ...
      // Wait for fail2ban to fully start after restart
    ...
    // After restart, if jail exists and is enabled, try to start it
    if (jailEnabled) {
      ...
      try {
        ...
        try {
          ...
          if (!fail2banActive) {
            console.warn(`[FILTER CREATE] ⚠️ Fail2ban service is not active, cannot start jail`);
            return res.json({
              success: true,
              filter: name,
              filterPath: filterPath,
              message: `Filter "${name}" created and jail enabled, but fail2ban service is not active. Please start fail2ban: sudo systemctl start fail2ban`,
            });
          }
        } catch (statusErr) {
        ...
        // Try to start jail (idempotent - won't fail if already started)
        try {
          const startResult = await runFail2banAction('start', name, true); // ignoreNOK = true
          
          // Wait a bit for jail to start
          await new Promise(resolve => setTimeout(resolve, 1000));
          
          // Verify jail is actually started
          ...
          if (activeJails.includes(name)) {
            ...
            // Jail didn't start - log warning but don't fail
          ...
          console.warn(`[FILTER CREATE] 💡 Jail is enabled in config - you can start it manually from the UI`);
        }
      } catch (autoStartErr) {
        console.warn(`[FILTER CREATE] ⚠️ Error during auto-start process: ${autoStartErr.message}`);
      }
    }
